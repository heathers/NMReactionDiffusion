...
int main()
{
    ...
    double ***lastx = d3tensor(1,n,1,n,1,n);
    double ***x = d3tensor(1,n,1,n,1,n);
    
    for (int i=1; i<=n; i++) {   //boundary conditions are six planes
      for (int j=1; j<=n; j++) {
      	 for (int k=1; k<=n; k++) {
   		xold[i][j][1]=0; xold[i][j][n]=0;
   		xold[i][1][k]=0; xold[i][n][k]=0;
   		xold[1][j][k]=0; xold[n][j][k]=0;
      	 }
      }
    }
	...
	for (int m=1; m<MAX_ITER; m++) {
	   ...
	   for (int i = 2; i < n; i++) {
    	      for (int j = 2; j < n; j++) {
	         for (int k = 2; k < n; k++) {
	   		x[i][j][k]=C/(6*C+1)*(x[i-1][j][k]+x[i+1][j][k]+x[i][j-1][k]
   ...
   printf("Time elapsed for %d cubic domain is %lf ms.\n", n, diff);
}
